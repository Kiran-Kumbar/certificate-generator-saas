...
import { FileText, Plus, Check, Play } from "lucide-react";
...
export default function SetupsPage() {
  ...
  const handleCreateOrUpdateSetup = async (e: React.FormEvent) => {
    ...
    try {
      const extractedKeys = Array.from(
        new Set(Array.from(programText.matchAll(/\{\{\s*(\w+)\s*\}\}/g)).map((m) => m[1]))
      );
      ...
      if (res.ok) {
        ...
        fetchData();
        alert(editingSetupId ? "Program Setup updated!" : "Program Setup created!");
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Certificate Setups</h1>
        <p className="text-sm text-slate-400 mt-1">Bind visual templates to program-specific text and dynamic variable definitions</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-fit">
          ...
          <form onSubmit={handleCreateOrUpdateSetup} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Setup Program Name</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Select Visual Template</label>
              <select
                value={templateId}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Program Certificate Text</label>
              <textarea
                value={programText}
                onChange={(e) => setProgramText(e.target.value)}
                rows={4}
                required
                className="w-full bg-slate-950 border border-slate-800 text-white rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 font-mono"
              />
        ...
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-base font-bold text-white">Configured Program Setups ({setups.length})</h2>

          {setups.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center text-slate-500">
              No certificate setups configured yet. Create a setup to enable individual and Excel bulk issuance.
            </div>
          ) : (
            <div className="space-y-3">
              {setups.map((setup) => (
                <div key={setup._id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-all flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      <h3 className="font-bold text-white text-base">{setup.name}</h3>
                      <span className="bg-indigo-500/10 text-indigo-400 text-[10px] font-semibold uppercase px-2 py-0.5 rounded">
                        Template: {setup.templateId?.name || "Assigned"}
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-2 line-clamp-2 bg-slate-950 p-2.5 rounded-lg border border-slate-800/60 font-mono">
                      {setup.programText}
                    </p>
                    ...
                    {setup.variables && setup.variables.length > 0 && (
                      <div className="flex items-center gap-1.5 flex-wrap mt-3">
                        ...
                        {setup.variables.map((v) => (
                          <span
                            key={v.key}
                            className="bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 text-[10px] font-mono px-2 py-0.5 rounded"
                          >
                            {"{{" + v.key + "}}"}
                          </span>
                        ))}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-2.5 py-1.5 rounded text-xs font-medium transition-colors"
                    >
                      View Details
                    </button>
                    <button
                      onClick={() => startEdit(setup)}
                      className="bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 px-2.5 py-1.5 rounded text-xs font-medium transition-colors"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDeleteSetup(setup._id)}
                      className="bg-red-500/10 hover:bg-red-500/20 text-red-400 px-2 py-1.5 rounded text-xs font-medium transition-colors"
                    >
                      Delete
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">{viewingSetup.name}</h3>
              <button onClick={() => setViewingSetup(null)} className="text-slate-400 hover:text-white text-sm">
                ✕
              </button>
            </div>
            ...
            <div>
              ...
              <div className="text-sm font-medium text-indigo-400">{viewingSetup.templateId?.name || "Assigned Template"}</div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {viewingSetup.variables?.map((v) => (
                  <div key={v.key} className="bg-slate-950 border border-slate-800 p-2 rounded text-xs">
                    <div className="font-semibold text-white">{v.label}</div>
                    <div className="font-mono text-[10px] text-indigo-400">{"{{" + v.key + "}}"}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
              ...
              <button
                ...
                className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-1.5 rounded-lg text-xs font-medium transition-all"
              >
                Close
              </button>
            </div>
